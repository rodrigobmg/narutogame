<?php
	...
	if($_POST && ($basePlayer->dono_equipe || $basePlayer->sub_equipe)) {
		$json			= new stdClass();
		$json->success	= false;
		
		$has_currency	= isset($_POST['pm']) && $_POST['pm'] == 1 ? $basePlayer->ryou >= 5000 : $basePlayer->coin >= 3;
		$target			= Recordset::query('SELECT * FROM bingo_book_equipe WHERE id_equipe=' . $basePlayer->id_equipe . ' AND id=' . (int)$_POST['target']);
		...
		if(isset($_POST['target']) && is_numeric($_POST['target']) && $target->num_rows && $has_currency) {
			...
			$mine_targets	= Recordset::query('
				...
				FROM
					bingo_book_equipe WHERE id_equipe=' . $basePlayer->id_equipe)->row();
			...
			Recordset::update('bingo_book_equipe', [
				'id_player_alvo'	=> [
					'escape'	=> false,
					'value'		=> '(' . $sql . ')'
				],
	...
	$records = new Recordset("
		...
		WHERE
			id_equipe=" . $basePlayer->id_equipe ." ORDER BY morto ASC");
?>
<style type="text/css">
	.imagem {
	...
	<?php foreach($records->result_array() as $record): ?>
		<div class="bingo_book_pagina" align="center" style="text-align: left">
			<?php for($f = 1; $f <= 1; $f++): ?>
				...
				?>
				<div class="imagem <?php echo $record['morto'] ? 'trans' : '' ?>" style="background-image: URL(<?php echo img('layout/bingobook/' . $player['id_classe'] . '.jpg') ?>)">
				</div>

<?php
	...
	if($_POST && $basePlayer->dono_guild) {
		...
		$has_currency	= isset($_POST['pm']) && $_POST['pm'] == 1 ? $basePlayer->ryou >= 20000 : $basePlayer->coin >= 5;
		...
		if(isset($_POST['target']) && is_numeric($_POST['target']) && $target->num_rows && $has_currency) {
			...
			Recordset::update('bingo_book', array(
				'id_player_alvo'	=> array(
					'escape' => false, 'value' => '(SELECT id FROM player WHERE id_graduacao >= 4 AND removido=0 AND DATEDIFF(CURDATE(), ult_atividade) < 3 ' . $where . ' ORDER BY RAND() LIMIT 1)'
				)
			), array(
				'id'				=> $_POST['target']
			));
...
<style type="text/css">
	...
	.nome {
		text-align: center;	
		background-image: URL('<?php echo img('bingobook/barra_nome.png') ?>');
		background-repeat: no-repeat; 
		background-position: center center;
		color: #FFF;
		font-size: 14px;
	...
	.titulo {
		text-align: center;	
		background-image: URL('<?php echo img('bingobook/barra_titulo.png') ?>');
		background-repeat: no-repeat; 
		background-position: center center;
		color: #FFF;
		font-size: 11px;
	...
	.b-wrap-right {
		background-image: URL('<?php echo img('bingo_book_r.png') ?>') !important
	}

	.b-wrap-left {
		background-image: URL('<?php echo img('bingo_book_l.png') ?>') !important
	}
...
<div id="bingobook">
	...
	<?php foreach($players->result_array() as $player): ?>
		<div class="bingo_book_pagina" align="center" style="text-align: left">
			<div class="imagem <?php echo $player['morto'] ? 'trans' : '' ?>" style="background-image: URL(<?php echo img('bingobook/' . $player['id_classe'] . '.jpg') ?>)">
			<?php if($player['morto']): ?>
				<img class="x" src="<?php echo img('bingobook/x.png') ?>" />
			<?php endif; ?>
			</div>
			<div class="nome" id="bingo-book-<?php echo $player['id'] ?>">
				<?php if($vip2): ?>
			...
			<?php if($vip1 || $vip2): ?>
				<?php ob_start() ?>
					...
					<?php endif ?>
					<?php if($vip2 && !$player['id_vila_atual']): ?>
			...
			<div class="recompensa">
				...
				<ul>
					<?php if($player['ryou']): ?>
					<li>Ryous: <?php echo $player['ryou'] ?></li>
					<?php endif; ?>
					
					<?php if($player['pt_treino']): ?>
					<li>Pontos de treino: <?php echo $player['pt_treino'] ?></li>
					<?php endif; ?>
					
					<?php if($player['exp']): ?>
					<li>Pontos de experiência: <?php echo $player['exp'] ?></li>
					<?php endif; ?>
				</ul>
				<?php if($basePlayer->dono_guild): ?>
				<br />
				<div align="center">
					<?php if($basePlayer->ryou >= 20000 && !$player['morto']): ?>
					...
					<?php if($basePlayer->coin >= 5 && !$player['morto']): ?>

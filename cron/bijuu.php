<?php
	...
	// Remove as bijuus
	Recordset::query("DELETE FROM player_item WHERE id_item IN(SELECT id FROM item WHERE id_tipo=23 )");
	...
	$bijuus = new Recordset("SELECT id FROM item WHERE id_tipo=23  ORDER BY RAND()");
	...
	while(true) {
		...
		while($count++ < 5) {
			...
			foreach($players->result_array() as $player) {
				// Se o cara tiver espada da nevoa, não ganha bijuu
				if(Recordset::query('SELECT id FROM player_item WHERE id_player=' . $player['id'] . ' AND id_item_tipo=39')->num_rows) {
					continue;
				}

				// Se o player passar no teste
				//if(rand(1, 100) <= $player['sorte_bijuu'] ) {
					$flag = new Recordset("SELECT bijuus FROM player_flags WHERE id_player=" . $player['id']);
					...
					if(Recordset::query("SELECT id FROM player_item WHERE id_item IN(SELECT id FROM item WHERE id_tipo=23 ) AND id_player=" . $player['id'])->num_rows) {
						continue;
					}
					// <--
					...
					if(!in_array($bijuu['id'], $flag)) {
						...
						Recordset::query("UPDATE player_flags SET sorte_bijuu=1, bijuus='" . addslashes(serialize($flag)) . "' WHERE id_player=" . $player['id']);

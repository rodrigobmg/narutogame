<?php
	...
	foreach($guilds->result_array() as $guild) {
		...
		$enemies		= Recordset::query("SELECT GROUP_CONCAT(id_player_alvo) AS e1 FROM bingo_book_guild WHERE id_guild=" . $guild['id'])->row();
		...
		Recordset::insert('bingo_book_guild', $insert);

...
(function($) { 
	...
	*/
	...
	/* Efeito personalizado slidedoom rs */
	...
	/* Efeito personalizado slidedoom rs */
	$.tools.addTipEffect("slideleft",   
     
    // opening animation 
    function() {  
        var opacity = this.getConf().opacity; 
        this.getTip().css({opacity:0}).animate({left: '-=25', opacity:opacity}, 300).show(); 
    },  
     
    // closing animation 
    function() { 
        this.getTip().animate({left: '-=25', opacity:0}, 300, function() {  
                $(this).hide().animate({left: '-=40'}, 0); 
        }); 
    } 
);

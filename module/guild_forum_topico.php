<?php
	...
	$rTopico = Recordset::query("SELECT * FROM guild_forum_topico WHERE id=" . (int)$id)->row_array();

	// Se o id do topico for e outra liva da negado (vai ser um mlagre isso aki acontecer .. mas por precaução)
	if($rTopico['id_guild'] != $basePlayer->getAttribute('id_guild')) {
		$redir_script = true;
		redirect_to("negado");
	}
?>
  <div class="titulo-secao"><p><?php echo t('guild_forum.g1')?></p></div>
	<?php if(isset($_GET['created']) && $_GET['created']): ?>
	    <?php msg(2,''.t('equipe_forum_topico.ef1').'',''.t('guild_forum.g3').''); ?>
    <?php endif; ?>   
    <table width="730" border="0" cellpadding="0" cellspacing="0">
          ...
          <td class="subtitulo-home"><table width="730" border="0" cellpadding="0" cellspacing="0">
                <tr>
                  ...
                  <td width="122" align="left"><b style="color:#FFFFFF;"><?php echo t('geral.topico')?>: <?php echo $rTopico['titulo'] ?></b></td>
    ...
    <?php
    	...
    	$qRespostas	= Recordset::query("
			...
			FROM
				guild_forum_topico_post a JOIN player b ON a.id_player=b.id
			
			WHERE
				a.id_guild_forum_topico=". (int)$id);
		
		if(!$qRespostas->num_rows) {
			echo "<i>".t('guild_forum.g4')."</i>";
		}
		
		foreach($qRespostas->result_array() as $r) {
    ...
    <form method="post" name="fResposta" id="fResposta" onsubmit="return false;" action="?acao=guild_forum_topico_responder">
    <input type="hidden" value="<?php echo encode($rTopico['id']) ?>" name="id" />
    <br />
    <br />
     <table width="450" style="margin:auto;" border="0" align="center" cellpadding="0" cellspacing="2">
        <tr>
          <th colspan="2" width="441" align="left"><b style="color:#FFFFFF"><?php echo t('geral.sua_resposta')?></b></th>
        </tr>
        <tr>
          <th colspan="2" align="left"><textarea name="msg_conteudo" cols="85" rows="7" id="msg_conteudo" onKeyDown="limitText(this.form.msg_conteudo,this.form.countdown,1000);" 
onKeyUp="limitText(this.form.msg_conteudo,this.form.countdown,1000);" maxlength="1000"></textarea></th>
        </tr>
        <tr>
        	<td align="left"><input readonly type="text" name="countdown" size="2" value="1000"> <?php echo t('geral.caracteres')?></td>

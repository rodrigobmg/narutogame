...
<?php
	...
	$where .= isset($_POST['nome']) && $_POST['nome'] ? " AND c.nome LIKE '%" . addslashes($_POST['nome']) . "%'" : "";
?>
<div class="titulo-secao"><p><?php echo t('titulos.rank_equipes'); ?></p></div>
<br />
      ...
      <tr >
      	...
      	<td width="190"  height="34" align="left"><select name="from" id="from">
			...
			$qTotal = Recordset::query("
                ...
                FROM 
                    ranking_equipe a JOIN equipe c ON c.id_player=a.id_player AND c.removido=0
		...
		<?php
            $query = Recordset::query("
                SELECT
                    ...
                    b.nome,
					b.id AS id_player,
					c.nome AS nome_equipe,
                    b.id_classe,
					c.id AS id_equipe,
                    (SELECT nome_".Locale::get()." FROM vila WHERE id=a.id_vila) AS nome_vila
                
                FROM 
                    ranking_equipe a JOIN player b ON b.id=a.id_player JOIN equipe c ON c.id_player=b.id
                
                WhERE c.removido=0
                     $where
                
                ORDER BY a.pontos DESC LIMIT {$_POST['from']}, 50");
            ...
            while($r = $query->row_array()) {
				...
				($r['id_equipe'] == $basePlayer->id_equipe) ? $cor="class='cor_roxa'" : "";
        ?>
       <tr <?= $cor ?>>
        <td width="50" align="center">&nbsp;</td>
        ...
        <td width="220" height="34" align="center">
			<a class="linkTopo <?php echo $posRanking ?>" style="font-size: 13px; <?php echo $posRanking2 ?>"  target="_blank" href="?secao=equipe_detalhe&id=<?= urlencode(encode($r['id_equipe'])) ?>"><?= $r['nome_equipe'] ?> - Lvl. <?php echo $r['level'] ?></a>
		</td>
        <td width="110" align="center"><?php echo $r['pontos'] ?></td>
        <td width="110" align="center"><img src="<?php echo img() ?>layout/bandanas/<?php echo $r['id_vila'] ?>.png" width="48" height="24" /></td>

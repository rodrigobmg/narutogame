...
class Opauth{
	...
	private function parseUri(){
		if(strpos($this->env['path'], '?') != -1) {
			$pathlen	= strlen(substr($this->env['path'], 0, strpos($this->env['path'], '?')));
		} else {
			$pathlen	= strlen($this->env['path']);
		}
		
		$this->env['request'] = substr($this->env['request_uri'], $pathlen - 1);
